...
<div class="container-fluid mt-3">
    <div class="card mb-3 border-0 shadow-sm" style="background-color:#f2f2f2;">
        <div class="card-body" style="background-color: #37B7C3; border-radius: 8px">
            <h2 class="m-0" style="color: #EBF4F6">Kegiatan Kokurikuler SMPN 3 Jember {{ optional($kelasSemester)->rombongan_belajar ? '| '.$kelasSemester->rombongan_belajar: '' }}</h2>
        </div>
    </div>
        ...
        <form id="p5bkFormData">
            ...
            <table id="example" class="table table-striped" style="width:100%">
                <thead>
                    ...
                    </tr>r_replace('-', ' ', $dimensi)) }}</td>
                        <td class="text-start">
                            <select name="capaian[{{ $dimensi }}]" class>                                                                           
                </thead>
                <tbody>
                    @foreach(['iman', 'kebhinekaan', 'mandiri', 'gotong-royong', 'kritis-kreatif'] as $index => $dimensi)
                    <tr>
                        <td class="text-start">{{ $loop->iteration }}</td>
                        <td class="text-start">{{ ucwords(stss="form-control capaian-select">
                                <option value="--">--</option>
